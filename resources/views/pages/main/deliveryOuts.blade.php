<x-app-layout>
    ...
    <main class="px-10 mt-10 relative">
        ...
        <button class="flex items-center gap-1 px-4 py-2 bg-green-700 rounded-md text-white font-medium text-sm"
            ...
            <img class="w-6" src="https://img.icons8.com/?size=100&id=oqWjYJSQSZAj&format=png&color=FFFFFF"
                alt="">
            Tambah Surat / Paket Keluar
        </button>

        <x-modal name="add-delivery" :show="false">
            <div class="p-5">
                <h5 class="font-semibold text-md">Tambah Surat / Paket Keluar</h5>

                <form class="mt-5" id="addDeliveryForm" onsubmit="handleAddDelivery(event)">
                    @csrf

                    <div class="mb-4">
                        <x-input-label for="resi" :value="__('Resi')" />
                        <x-text-input id="resi" class="block mt-1 w-full" type="text" name="resi" required
                            autofocus />
                    </div>
    ...
    <x-modal name="edit-delivery" :show="false">
        <div class="p-5" x-data="{
            ...
            <form class="mt-5" id="editDeliveryForm" onsubmit="handleEditDelivery(event)">
                ...
                <div class="mb-4">
                    <x-input-label for="edit_resi" :value="__('Resi')" />
                    <x-text-input id="edit_resi" class="block mt-1 w-full" type="text" name="resi" required
                        x-bind:value="delivery?.resi" autofocus />
                </div>

                <div class="mb-4">
                    <x-input-label for="edit_delivery_date" :value="__('Tanggal Pengiriman')" />
                    <x-text-input id="edit_delivery_date" class="block mt-1 w-full" type="date" name="delivery_date"
                        required x-bind:value="delivery?.delivery_date" autofocus />
                </div>
                ...
                <div class="mb-4">
                    ...
                    <x-text-input id="edit_receiver" class="block mt-1 w-full" type="text" name="receiver"
                        required x-bind:value="delivery?.receiver" autofocus />
                </div>
    ...
    <script>
        ...
        function handleAddDelivery(event) {
            ...
            setLoading(submitButton, true, 'Tambah Surat / Paket Keluar', 'add');

            $.ajax({
                url: "{{ route('delivery-outs.store') }}",
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
                ...
                complete: function() {
                    setTimeout(() => {
                        setLoading(submitButton, false, 'Tambah Surat / Paket Keluar', 'add');
                    }, 500);
                }
            });
        }
        ...
        function handleEditDelivery(event) {
            ...
            $.ajax({
                url: `/delivery-outs/${PenggunaanId}`,
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
        ...
        function handleDeleteDelivery(event) {
            ...
            setLoading(submitButton, true, 'Hapus Surat / Paket Keluar', 'delete');
            ...
            $.ajax({
                ...
                complete: function() {
                    setTimeout(() => {
                        setLoading(submitButton, false, 'Hapus Surat / Paket Keluar', 'delete');
                    }, 500);
                }
            });
        }
